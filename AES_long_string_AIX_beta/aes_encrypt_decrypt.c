...
int main(int argc , char* argv[]){
	...
	FILE *encrypt_file;
	encrypt_file = fopen("encrypt_file","wb");
...
////////////////////////////encrypt end/////////////////////////////////
		...
		/*unsigned char map_buf[32];
		for(i=0;i<32;i+=2){
			sprintf(&map_buf[i],"%02x",map[(i%8)/2][i/8]);
		}
		fwrite(map_buf,sizeof(char),32,encrypt_file);*/
	...
	fclose(encrypt_file);
	printf("encrypt_file is now at ./encrypt_file\n");
	
	/*printf("after encrypt:\n");
	print_file("encrypt_file.txt");*/
	
	
	
	FILE *decrypt_file;
	decrypt_file = fopen("decrypt_file","wb");
	
	encrypt_file = fopen("encrypt_file","rb");
	fseek(encrypt_file,0,SEEK_END);
	round = ftell(encrypt_file)/16;
	...
	for(round_count=0;round_count<round;round_count++){
		...
		fread(input,sizeof(char),16,encrypt_file);
